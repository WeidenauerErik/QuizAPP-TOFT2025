...
export function QuizPage({user, quizId, onBack}: QuizPageProps) {
    ...
    const loadQuiz = async () => {
        ...
        setError('');
        try {
            const response = await fetch(`https://localhost/getQuiz?quiz_id=${quizId}`);
            if (response.ok) {
    ...
    const handleSubmit = async () => {
        ...
        quiz.questions.forEach((question, index) => {
            ...
            if (selectedIndex === -1) return; // noch keine Auswahl
        ...
        try {
            await fetch('https://localhost/submitQuiz', {
                method: 'POST',
                headers: {
            ...
            <div className="min-h-screen p-6 flex items-center justify-center">
                <div className="w-full max-w-md">
                    <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 border border-white/20 text-center">
                        <div
                            className="bg-gradient-to-br from-green-400 to-emerald-500 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6">
